...
class Decoder(nn.Module):
    def __init__(self, in_channels, channels, out_channels):
        ...
        self.spatial_gate = SpatialGate(out_channels)
        self.channel_gate = ChannelGate(out_channels)
        self.alpha1 = nn.Parameter(torch.FloatTensor([1]))
        self.alpha2 = nn.Parameter(torch.FloatTensor([1]))
        self.alpha3 = nn.Parameter(torch.FloatTensor([1]))
...
class UNetResNet34(BasicModule):
    def __init__(self, pretrained=False):
        ...
        self.logitNew = nn.Sequential(
            nn.Conv2d(320, 64, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64,  11, kernel_size=1, padding=0),
        )
        self.linearAttenNew = nn.Sequential(
            nn.BatchNorm1d(512),
            nn.Linear(512,256),
            nn.ReLU(inplace=True),
            nn.Linear(256,9),
...
#         ], 1)
        ...
        f = F.dropout(f, p=0.5)
        logitNew = self.logitNew(f)
        empty_pro = F.adaptive_avg_pool2d(e5, output_size=1)
        empty_pro = F.dropout(empty_pro, p=0.50)
        empty_pro = empty_pro.view(empty_pro.size(0),-1)
        empty_pro = torch.squeeze(self.IsEmpty(empty_pro))
        empty_pro_reshape = F.softmax(empty_pro,1)
        empty_pro_reshape = empty_pro_reshape.view(*empty_pro_reshape.size(),1,1)
        
        return torch.sum(logitNew*empty_pro_reshape,1,True),empty_pro
